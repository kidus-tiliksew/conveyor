...
execution. `create_task` accepts an optional title, repository, task context and
escalation level, plus a required workspace-scoped idempotency key. When title
is omitted, trusted control-plane AI generates it from the task context before
the durable task is created (§21.24). The operation creates
...
*End of specification. v1.25 accepted July 18, 2026; all prior amendments remain in force, and §21.24 adds trusted AI-generated task titles while preserving explicit-title compatibility. Subsequent changes proceed by amendment with version bumps.*
