...
**Specification — v1.31**
**Date:** July 20, 2026
**Status:** Accepted — **Beta achieved July 15, 2026** (§19 exit criterion met); execution settings are contextual (§21.18), provider-neutral reasoning effort is available for review seats and implementation (§21.19), harness MCP transport is explicit (§21.20), worker-attempt recovery is bounded and audited (§21.21), portable GitHub review projection plus headless-worker reliability are defined by §21.22, terminal review-round recovery is defined by §21.23, task titles are generated by trusted control-plane AI (§§21.24–21.25), reconnect-safe workers plus interrupted-seat recovery are defined by §21.26, named execution setups with per-task selection are defined by §21.27, in-process USD cost accounting is removed by §21.28, environment-backed MCP attachment is defined by §21.29, and merge readiness, conflict-fix dispatch, and refresh review are defined by §21.30
...
*End of specification. v1.31 accepted July 20, 2026; all prior amendments remain in force, and §21.30 adds merge readiness, conflict-fix dispatch, and refresh review. Subsequent changes proceed by amendment with version bumps.*
