...
**Specification — v1.41**
**Date:** July 28, 2026
**Status:** Accepted — **Beta achieved July 15, 2026** (§19 exit criterion met); execution settings are contextual (§21.18), provider-neutral reasoning effort is available for review seats and implementation (§21.19), harness MCP transport is explicit (§21.20), worker-attempt recovery is bounded and audited (§21.21), portable GitHub review projection plus headless-worker reliability are defined by §21.22, terminal review-round recovery is defined by §21.23, task titles are generated by trusted control-plane AI (§§21.24–21.25), reconnect-safe workers plus interrupted-seat recovery are defined by §21.26, named execution setups with per-task selection are defined by §21.27, in-process USD cost accounting is removed by §21.28, environment-backed MCP attachment is defined by §21.29, merge readiness, conflict-fix dispatch, and refresh review are defined by §21.30, execution modes are removed in favor of a per-task hold by §21.31, pinned harness snapshots re-resolve on queue re-entry by §21.32, the spec stage is delegated over MCP with triage briefs and non-normative spec diagrams by §21.33, task cancellation, setup re-freeze on operator recovery, and the stalled-task tray are defined by §21.34, audited task execution-setup reassignment for remaining work is defined by §21.35, the setup-reassignment active-attempt exclusion is scoped to executing attempts by §21.36, canonical lifecycle state machines with normative transition tables are defined by §21.37, all task mutation is serialized through the command plane by §21.38, task bodies are GitHub-flavored Markdown under §21.39, and worker-side first-activity liveness is defined by §21.40
...
execution. `create_task` accepts repository and task context, plus a required
workspace-scoped idempotency key; title is not an intake field. Trusted
control-plane AI generates the persisted title from the task context before
the durable task is created (§21.25). The operation creates
...
### 21.40 v1.41 — Worker first-activity liveness (July 28, 2026)
...
*End of specification. v1.41 accepted July 28, 2026; all prior amendments remain in force, §21.37 defines the canonical lifecycle state machines with normative transition tables, §21.38 defines the serialized task command plane, §21.39 defines task bodies as GitHub-flavored Markdown, and §21.40 defines worker-side first-activity liveness. Subsequent changes proceed by amendment with version bumps.*
