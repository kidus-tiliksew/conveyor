...
| **Prompt/policy pack** | The versioned bundle of role prompts shipped with the platform (proto-pack since Phase 3). Full pack versioning with shadow-run gating is Phase 10 scope (§2.2). |
...
Phase 10 scope, deferred until the transcript corpus exists to replay
...
criteria await the Phase 11 verification agent (§12). (3) Verdicts attach
...
### 7.2 Multi-repo coordination *(deferred design, Phase 11)*
...
1); revisions to either artifact happen in the same conversation.
**Sessions declare a goal artifact** (§21.57): `requirement`,
`blueprint`, or `open` — set at creation, steering the agent's
finalize target so a requirement-drafting conversation does not skip
to a blueprint; the goal is advisory to the agent and enforced as a
finalize-time check for the non-`open` goals. Session titles derive
from the artifact produced, not from a static label. The
...
**The planning agent reads code, revision-pinned (§21.50).** Beyond the
...
budgeted — the per-call cap is
`control_plane.planning.exploration_output_tokens` (default 10 000,
validated positive, hot-reloaded like all §2.1 configuration; the
soft session exploration budget defaults to fifteen times it,
§21.52) — with annotated truncation, and shaped for **cross-model
legibility** — the planning model is configurable, so tool contracts
never assume a model family.

**The planning model is configurable.** Planning joins triage in the
control-plane execution settings
...
provenance for the lineage graph and, later, the Phase 10 flywheel
...
3. **The verification agent** *(Phase 11, demand-triggered)*: scripted
...
**document-first** (§21.57): the corpus list beside the document as
the canvas — REQ-n statements, prose, version history and diffs,
confirmation, staleness — with the **planning assistant docked as a
sidebar scoped to that document**, offering guided actions (draft,
revise, Q&A, plan work → a blueprint session in the requirement's
context with the `serves` proposal) rather than a detached chat; every
revision remains a proposed version the operator confirms — the
sidebar is the editor, and freehand editing stays out because it would
bypass the §4.2 propose→confirm audit; the
**planning surface**
(Phase 6.2, reshaped by §21.57) — the session list and free-form chat
remain for goal-`open` and blueprint-only sessions, with streamed
replies, tool-activity markers, and attachments, ending in the
ordinary spec-gate hand-off; and **dependency affordances** (Phase 6.1) —
...
**Operating surfaces (§21.61).** The dashboard presents exactly the
operating surfaces: Board (the pipeline lens, filterable by updated
date, served requirement, and governing design, defaulting to recent
activity), Tasks (the list-first management surface: paginated,
filterable, creation-in-view, rows opening a permalinked detail
panel), Workspace, Requirements and System Design (category tree,
document canvas, and one per-document attention surface that is the
sole document-side rendering of drift, staleness, and pending-version
signals), Monitor, and Settings. One carve-out (§21.62): a task's own
detail additionally renders that task's own unresolved System Design
proposals on its attached documents, with their confirm action; drift
and staleness stay document-side only. Task, requirement, and design
detail expose an on-demand lineage explorer — grouped related entities
over the §16 read API, not a visualization. The in-product planning chat surfaces
(§21.57) and the Blueprint-history navigation entry (§21.49) are
parked: withdrawn from presentation with their machinery, records,
and deep links retained; the §21.60 local surface carries planning
while parked. §6.3's barred fields remain barred from every surface.
...
3. **Attribution stays visible** in the per-task timeline. The
   aggregate cost dashboard is Phase 11, observational only; nothing
   revives spending enforcement without a new accepted amendment.

---

## 15. Memory and self-improvement *(Phase 7 demand-triggered; Phase 10)*

### 15.1 Recall *(Phase 7, demand-triggered — §21.55)*

**There is no memory store.** A store of volunteered knowledge rows is
the features-tree anti-pattern with embeddings: free-standing claims no
pipeline stage re-verifies, in a factory whose alignment machinery
(requirement staleness, drift write-back, citations, lineage) exists
precisely so knowledge cannot silently rot. Knowledge worth keeping is
**promoted into a maintained artifact** — a requirement revision
(§4.2), a `.conveyor/hints.yaml` entry, or a pack-prompt change via the
Phase 10 flywheel's human-reviewed proposals — never parked in a store.
Architecture decisions are requirements; conventions are repo-resident
hints; lessons live in the verdicts, redirect feedback, and transcripts
the factory already records.

What Phase 7 may add, when operating Phase 6.3 demonstrates the need,
is **recall**: read-only retrieval over artifacts that already exist —
the requirement corpus, approved specs, review verdicts and redirect
feedback, planning transcripts, and lineage links — for questions the
graph holds but edges don't reach ("have we dealt with this before?").
Hybrid keyword/vector retrieval (Postgres + pgvector) as a secondary
index over those artifacts, never a primary structure and never
writable: `get`-side MCP tools only, on the §17.4 server per the
§21.12 transport decision; the worker stays uninvolved; there is no
`store_memory` and no memories table. The trigger is evidence, not the
calendar: collected instances where an agent lacked knowledge the
factory held but could not reach through §4.2 item 4's graph-walk
context assembly.

### 15.2 Self-improvement engine *(Phase 10)*

Consumes transcripts, reason
codes, bounce histories, and telemetry; produces human-reviewable
proposals (never auto-applied): role-prompt and policy edits,
requirement or hints amendments (the promotion path of §15.1),
gate-default and hold-usage graduation, and pack versioning
...
**Permitted exception:** the Phase 10 analysis side of the
...
- **Prompt-injection posture:** task bodies, issue content, PR
  comments, repository content read by planning exploration (§21.50),
  uploaded documents, and lineage-derived context items (§21.59) are
  untrusted data — framed with origin and fenced wherever they enter a
  prompt, never treated as instructions. Headless harness permission
  grants are
...
architecture rather than enterprise add-ons. Phase 9 ships the
...
12 is the genuinely enterprise layer built atop it: SSO/OIDC and SAML,
...
| **7** | Recall: read-only retrieval over the requirement/spec corpus, verdicts, transcripts, and links; pgvector as secondary index; get-side MCP tools only — **no memory store, no memory rows** (§15.1, §21.55) | Demand-triggered — evidence from operating 6.3 |
| **8** | Desired-state document model (§21.58): **8.1** requirements v2 (REQ→AC structure, user-story framing) + product overview uploads (markdown, informative, promoted), **8.2** System Design corpus + decision records (`DEC-n`), **8.3** task-centric delivery — §4.1 fences and decomposition materialization retire while the spec stage survives re-contented as the plan stage (gate semantics unchanged); tasks carry attached context and planning finalize proposes bundles, **8.4** Tasks view + lineage/staleness simplification + historical blueprint data preserved as record | Accepted (§21.58) — follows Phase 6 closure |
| **9** | Deployment & multi-user (§21.54): **9.1** embedded worker (§6.6), **9.2** identity, grants, and credentials (§10, §16, §17.5), **9.3** delivery tiers — GitHub optional (§11), **9.4** packaging & first-run (§6.5, §17.1–§17.2) | Accepted (§21.54, resequenced §21.58) — follows Phase 8; 9.1 may land earlier |
| **10** | Flywheel: transcript mining, self-improvement proposals, gate/hold graduation, pack versioning with eval rig and shadow runs (§15.2) | Consumes the accumulated corpus |
| **11** | Managed-execution reintroduction, demand-triggered: verification agent (§12), K8sRunner, multi-repo worktree sets and linked-PR gating (§7.2), aggregate cost dashboard | Demand-triggered |
| **12** | SSO/OIDC and SAML, SCIM, RBAC beyond two roles, HA/backup hardening (§18.1) | Demand-triggered |
...
growth. **Phase 8 (deployment & multi-user) follows Phase 6 directly**
(§21.55): Phase 7 is demand-triggered recall, built only when operating
6.3 produces evidence of reachability misses — it holds no slot in the
sequence. Phase 8 runs 8.1 → 8.4, except 8.1 (embedded worker), which
touches only §6.4 supervision and may land earlier — during late
Phase 6 (§21.54). This section is authoritative for scope
and ordering.
...
### 21.50 v2.10 — Planning repo exploration and configurable planning model (July 31, 2026)

Adopted after the Phase 6.2 batch landed, from the observation that
in-product planning was corpus-grounded but code-blind: the planning
agent could read what the system is supposed to do (requirements,
approved specs, links) but not what the code actually does, so
blueprints drafted in-product would be systematically shallower than
plans made in an operator's own coding agent — the strongest original
argument against owning planning in-product. Design informed by a
review of the open-source Codex CLI: Codex exposes *no* dedicated read
tools (all exploration is one sandboxed shell plus prompt idioms), so
its mechanism cannot be copied without an execution environment — but
its shapes can: ripgrep-format search results, ~10k-token output
budgets with annotated middle truncation for search, and its plan-mode
prompt discipline. Claude Code's typed-tool conventions (line-numbered
reads, deterministic pagination) supply the reading side. Four changes:

1. **Four revision-pinned exploration tools (§9).** `list_files`,
   `read_file`, `grep`, and `history`, implemented with git plumbing
   over the §8.1 fetch-only bare-clone cache — no working tree, no
   shell, no sandbox, no writes. The session pins one revision at
   start (no `ref` parameter anywhere); the SHA is recorded on the
   session row and becomes a lineage fact — a blueprint's plan saw a
   specific code state, which the §4.2 alignment layer can later check
   staleness against. Reading for planning is not execution; the §21.4
   delegation boundary is untouched, and repo content entering the
   planning prompt carries the §18 untrusted-data posture (safe
   because everything the agent produces exits through operator
   confirmation and the §13.1 spec gate).

2. **Output contracts are cross-model, budgeted, and deterministic.**
   Reads are line-numbered with header context and deterministic
   pagination (never middle-truncated — the model asked for a
   specific region); search output is ripgrep-shaped
   (`path:line:text`) with per-call token caps and an explicit
   truncation annotation carrying the original size; listings cap
   with a "narrow your query" hint; a soft per-session exploration
   budget degrades caps gracefully in long chats. Conventions are
   chosen for cross-model legibility — the overlap of the Codex and
   Claude Code ecosystems — and no tool contract may assume a model
   family.

3. **The planning model is operator-configurable.** Planning joins
   triage in the control-plane execution settings
   (`control_plane.planning.{model, effort, timeout}`), workspace
   default plus per-session override constrained to a
   workspace-curated `planning_models` allowlist (curated like review
   seats — auditable, UI-honest). The in-process integration's
   existing provider routing applies unchanged. Sessions record
   `{model, effort, pinned SHA}`: provenance for lineage now, flywheel
   material for Phase 8 later.

4. **Planning prompt discipline enters the pack (§2.2).** From Codex's
   plan mode, adapted: explore first, ask second (at least one
   targeted exploration pass before any clarifying question); never
   ask what the environment can answer; parallelize independent
   reads; cite `file:line` evidence in blueprints; finalized
   artifacts are decision-complete and revisions are complete
   replacements. Prompt guidance lives in the versioned pack, not in
   tool schemas, so per-model steering is a pack concern.

Out of scope, restated: shell or exec tools for planning; writes of
any kind; build/test execution ("does it reproduce" is a pipeline
task, not a planning capability); graph-walk context assembly (Phase
6.3, which will complement exploration by telling the agent where to
look).
...
### 21.52 v2.12 — Exploration output cap is planning configuration (July 31, 2026)

Refinement to §21.50 change 3, same day, before implementation,
adopted from comparing harness budget designs: Codex's output budgets
are per-model policy, not constants — and since the planning model is
operator-configurable, a fixed exploration cap fits one model family
and pinches another. One change: `control_plane.planning` gains
**`exploration_output_tokens`** — the per-call output cap for the §9
exploration tools. Default 10 000, validated positive, hot-reloaded
like all §2.1 configuration, recorded on the session alongside model
and effort. The soft session exploration budget defaults to fifteen
times the per-call cap so the two scale together with one knob.
Truncation annotations and every other §21.50–§21.51 output contract
are unchanged.

---

### 21.53 v2.13 — Attempt-aware worker recovery presentation (July 31, 2026)

The worker recovery contract in §21.21 had durable retry metadata but not the
identity needed to keep causes from separate executions apart, and its generic
failure text made provider exhaustion indistinguishable from an ordinary child
exit. This amendment adds presentation-safe audit facts without changing any
lifecycle or recovery transition:

1. **Attempt identity is durable.** Every successful claim starts an immutable
   `attempt_id`. The claimed work-order projection and claim event carry it;
   release, child-failure, stall, expiry, timeout, and operator-recovery audit
   records retain the identity of the attempt they close or act on. The
   projection separately retains `last_attempt_id` after active claim fields
   are cleared. Historical records without either field remain visible as
   legacy activity and are never associated with a later identified attempt.
2. **Child-failure category is durable and provider-neutral.** Worker failure
   classification records `failure_category` on closing events and
   `last_failure_category` on the work-order projection. The initial category
   is `provider_usage_limit`. Empty or unrecognized values are generic failures;
   their original message, captured detail, exit status, retry count, retry
   time, and suppression metadata remain authoritative and inspectable.
3. **One attempt owns one cause.** Claim starts an attempt; release, expiry,
   cancellation, timeout, stall, or child failure closes it. Cause, retry, and
   recovery presentation may be joined only through the same attempt identity.
   A later dirty-primary-checkout release therefore supersedes an earlier
   provider-limit failure as the current blocker without erasing either audit
   record.
4. **The latest actionable attempt is the current-state authority.** A pending
   bounded retry shows `Retrying in …` and no manual recovery action. A
   retry-suppressed `provider_usage_limit` offers `Retry implementation` after
   the pending retry has ended. A dirty-primary-checkout release first tells the
   operator to resolve the primary checkout and does not emphasize or enable
   retry while that safety gate remains unresolved. §21.21 retry counts,
   backoff, queue eligibility, recovery authorization, and idempotency are
   unchanged.
5. **History is attempt-grouped and progressively disclosed.** Concise attempt
   summaries keep resolved or retried failures neutral. Lease renewals, queue
   bookkeeping, and comparable low-signal records remain in the complete audit
   history behind a semantic `Show technical activity` disclosure. Captured
   child output remains behind its own technical-details disclosure; a specific
   categorized cause takes precedence over the generic harness-exit wrapper.
6. **Attention is semantic and accessible.** Only the current unresolved
   blocker receives the strongest warning treatment, with text in addition to
   color or icon. Summary, history, and actions follow reading order;
   disclosures are keyboard operable with visible focus; and a transition from
   running to paused or needs-attention is announced through a polite live
   region.
...
### 21.55 v2.15 — No memory store: Phase 7 becomes demand-triggered recall (August 1, 2026)

Adopted from operating Phase 6: the factory's alignment machinery makes
a centralized memory store redundant — and worse, doctrinally wrong. A
store of volunteered knowledge rows is the features-tree anti-pattern
with embeddings: free-standing claims with no maintenance loop, in a
system whose requirement staleness, drift write-back, `REQ-n`
citations, and lineage exist precisely so knowledge cannot silently
rot. Everything the memory store was scoped to hold already has a
maintained home: architecture decisions are requirement documents
(versioned, confirmed, staleness-surfaced); conventions are
repo-resident (`.conveyor/hints.yaml`, agent notes); lessons are the
verdicts, redirect feedback, bounce histories, and transcripts the
pipeline records as first-class artifacts. Four changes:

1. **The knowledge lifecycle is promotion, not parking (§15.1).**
   Knowledge worth keeping is promoted into a maintained, verified
   artifact — a requirement revision, a hints entry, or a pack-prompt
   change through the Phase 9 flywheel's human-reviewed proposals.
   `store_memory`, memory rows, and a memories table are **rejected,
   not deferred** — do not build them under any phase.

2. **Phase 7 rescopes to recall, demand-triggered.** What may still be
   built is read-only retrieval over artifacts that already exist
   (requirement corpus, approved specs, verdicts and redirect
   feedback, planning transcripts, links) for questions edges don't
   reach. pgvector remains a secondary index over those artifacts,
   never a primary structure and never writable; `get`-side MCP tools
   only, on the §17.4 server per the §21.12 transport decision. The
   trigger is evidence from operating 6.3's graph-walk context
   assembly — collected reachability misses — not the calendar.

3. **Phase 8 follows Phase 6 directly (§19).** Deployment &
   multi-user's "follows the Phase 7 memory store" ordering from
   §21.54 is superseded; Phase 7 holds no slot in the sequence. The
   8.1 embedded worker's may-land-earlier latitude now reads "during
   late Phase 6."

4. **§15.2's flywheel outputs drop "memory entries"** in favor of the
   promotion path: requirement or hints amendments alongside
   role-prompt and policy edits. Phase 9's scope is otherwise
   unchanged, and remains the only producer of distilled lessons —
   with a human reviewing every promotion.

Unchanged: §21.12's MCP transport decision (now governing recall's
get-side tools), the worker's non-involvement, per-role context
budgets if recall ships, and every Phase 6/8/9 boundary.

---

### 21.56 v2.16 — Attempt-history presentation retired (August 1, 2026)

§21.53 change 5's attempt-grouped history section is retired from the
task-detail presentation. Operating it showed the per-attempt list
added no operator value: the §21.53 current-state card already carries
the one actionable summary (state, blocker, retry, next action), and
the complete audit history — including captured child failures and
their categorized causes — remains reachable behind the `Show
technical activity` disclosure. A second, per-attempt grouping of the
same records was clutter between those two layers. Two changes:

1. **Change 5's attempt-grouping clause is retired.** The surviving
   §21.53 presentation contract is: the current-state card is
   authoritative for the latest actionable attempt; low-signal records
   and captured child output live behind the technical-activity
   disclosure with a specific categorized cause taking precedence over
   the generic harness-exit wrapper. Change 6 (semantic, accessible
   attention) and changes 1–3 (durable attempt identity, failure
   categorization, terminal-closure capture — the data contract) are
   untouched.

2. **Process note, recorded for the change record's honesty.** The
   deletion landed as task `260801-7ae4fa` (PR #207) *before* this
   amendment — code and spec disagreed for one day. The operator
   confirmed the direction and this entry ratifies it; the rule stands
   that presentation mandates, like every normative clause, retire by
   amendment rather than by diff.

---

### 21.57 v2.17 — Document-first planning: the requirement is the canvas (August 2, 2026)

Adopted from operating the first real planning sessions. Two defects
surfaced together: the chat was the primary surface with documents as
byproducts (the operator could not see or iterate on *the document*,
which is the artifact that matters), and nothing steered a session's
artifact type — the first 6.3 session, titled "New requirement,"
explored well and then finalized a blueprint with no requirement and
no serves link. The chain the factory exists to serve — requirement →
blueprint → work orders, each link AI-assisted — needs the document at
the center and the assistant beside it. Three changes:

1. **The Requirements view becomes document-first (§13.3).** Corpus
   list beside the document canvas (statements, prose, version
   history and diffs, confirmation, staleness), with the planning
   assistant docked as a sidebar whose session is scoped to that
   document. Guided actions replace the blank prompt: draft, revise,
   Q&A, and plan-work (opens a blueprint session in the requirement's
   context, proposing the `serves` link). The free-form planning page
   remains for goal-`open` and blueprint-only sessions.

2. **Deliberately, no freehand editor.** Every revision is a proposed
   version the operator confirms (§4.2 item 1), so the AI-mediated
   sidebar *is* the correct editor: the safe path and the convenient
   path coincide, and the propose→confirm audit trail stays whole.
   Manual text editing of requirement documents is rejected, not
   deferred.

3. **Sessions declare a goal artifact (§9).** `requirement`,
   `blueprint`, or `open`, set at creation; the goal steers the
   agent's finalize target and is enforced at finalize time for
   non-`open` goals. Session titles derive from the artifact produced.
   Presentation and steering only — no data-model change beyond the
   goal column; the §21.4 boundary, gates, and §21.46/§21.49 bars all
   stand.

---

### 21.58 v2.18 — Phase 8: the desired-state document model (August 3, 2026)

Adopted from operating Phase 6 end to end and a comparative study of
contemporary factory document models. The finding: Conveyor's
"blueprint" and the industry's were false cognates — ours was a
per-change transition plan, while the durable technical counterpart of
requirements (how the system is built, as living truth) had no factory
home at all, and the per-change spec document was doing three jobs
that durable documents and a lighter plan do better. Conveyor moves
from a **migration model** (a gated transition document per change) to
a **desired-state model**: durable documents are the truth —
requirements for intent, System Design for mechanism — and tasks are
the reconciliation work that moves code toward them, each carrying its
own lightweight execution plan. Nine changes:

1. **Four durable document tiers**, each with its own maintenance
   loop, under a normative/informative doctrine recorded in §4.2:
   **product overview** — informative, markdown-only uploaded
   documents (flexible named set; supersession-with-diffs on
   re-upload; document nodes and session `consulted` edges in the
   graph; top planning-context slot; §18 posture; citations and
   staleness never target them); **requirements** — normative intent,
   factory-native: REQ entries gain optional user-story framing and
   nested acceptance criteria (`REQ-n` → `AC-n.m`), additive over the
   existing statement corpus, making requirements independently
   verifiable state; **System Design** — normative mechanism:
   factory-resident versioned markdown documents in operator-named
   categories (SDD, architecture, low-level design, database design,
   API contracts, …), proposed by agents and operators through the
   same propose→confirm machinery as requirements, freshness-checked
   (merges touching a component without touching its documents are a
   drift signal; review may demand document updates in the same
   change); **decisions** — `DEC-n`, append-only with supersession,
   machinery-proposed from planning and implementation deliberation,
   human-confirmed, citable in code and documents alongside `REQ-n`.
   Informative content that needs enforcement is **promoted** into a
   normative tier with a section-anchored `derived_from` link — the
   §21.55 promotion doctrine's concrete mechanism.

2. **The per-change spec artifact retires; its functions relocate.**
   The §4.1 fence formats, the blueprint noun, and §21.46
   decomposition materialization are superseded when Phase 8.3 lands
   — the spec *stage* itself survives, re-contented as the plan stage
   of change 4. Relocations:
   acceptance criteria → the cited requirements' ACs; the approach →
   System Design deltas, revised *before* delivery so documents move
   first and code follows; decomposition and ordering → direct
   task-set creation over the unchanged §6.3/§8.3 dependency
   machinery; the change rationale → task description, attached
   context, and the planning transcript; the per-change plan → the
   execution plan of change 4. Historical blueprint tasks, events,
   and lineage nodes are preserved as record, never rewritten.

3. **Tasks are the sole transition object, and they carry context.**
   Intake on every surface gains explicit context attachment: the
   requirements a task serves and the System Design documents that
   govern it, alongside artifact attachments. The `serves` edge moves
   to task level.

4. **Execution plans ride the existing spec-stage machinery,
   re-contented.** The plan stage behaves exactly as the spec stage
   does today: when the §13.1 first toggle (renamed from spec
   approval to plan approval, workspace default with per-task
   override) or routing calls for it, a stage-typed work order is
   dispatched before implementation; the claiming agent — not
   necessarily the eventual implementer — reads the task's attached
   context and submits a markdown execution plan: approach, files
   touched, ordering, risks, and explicit done-criteria. Plans are
   versioned; gate approval, redirect-to-revision, and auto-approval
   with the gate off are byte-compatible with today's spec-gate
   semantics, and with the gate off routing may skip the plan stage
   entirely, exactly as triage routes direct-to-implement now.
   `submit_plan` succeeds `submit_spec` on the §17.4 surface (wire
   compatibility handled at implementation). What changes is content
   only: no §4.1 fences, no decomposition — fan-out stays
   planning-session territory, and an implementing agent that
   discovers oversized scope reports back through the existing
   check-in machinery rather than minting children. A task that
   reaches review with neither cited requirement ACs nor a plan
   carries its statement of done in the task description.

5. **The pre-execution human gates, restated.** Planning finalize
   proposes a **bundle** — document revisions plus a task set with
   dependencies — and the operator's approval of that bundle is
   consent to create the work. The plan gate (the first §13.1 toggle,
   semantics unchanged from spec approval) is consent to implement.
   The merge gate is unchanged. Agents never mint executable work or
   land changes without a human moment while the respective toggles
   are on.

6. **The graph simplifies.** The chain becomes requirement → task →
   code → evidence, with System Design and overview documents as
   context nodes (`governs`, `consulted`, `derived_from` edges) and
   plans as task-scoped nodes. Staleness walks delivery edges at task
   level. Blueprint and blueprint-version node types persist for
   historical subgraphs only.

7. **A Tasks view joins the surfaces** — list-first management with
   filters, dependency and blocking columns, child rollups, and
   attached-context links. It imports the *view*, not the fields:
   priority, assignee, and declared phases remain barred (§6.3,
   §21.31, §21.46) — one queue, derived state, workers claim.

8. **Explicitly preserved**: both §13.1 gates (one relocated, neither
   removed); the §6.3/§8.3/§21.47 dependency machinery; the §17.4 MCP
   lifecycle with `submit_plan` succeeding `submit_spec`; planning sessions, exploration, and
   budgets (§21.50–§21.52); REQ-n citations and review contracts
   (strengthened by requirement-level ACs and plan done-criteria);
   evidence gating; the lineage projector and conformance discipline;
   the append-only event corpus.

9. **Resequencing.** This is Phase 8, entered only after Phase 6
   closes (residual wave, live rebuild, closing amendment).
   Deployment & multi-user (§21.54) moves to Phase 9 unchanged in
   content — its embedded worker may still land early. Flywheel → 10,
   managed execution → 11, enterprise → 12. Phase 7 recall remains
   demand-triggered.

Working breakdown in docs/phase8-plan.md. Out of scope, restated:
priority/assignee/phase fields; freehand editing of any normative
tier; PDF/Word ingestion; repo-resident System Design (reconsidered
only after operating the factory-resident corpus); agent-minted child
tasks from implement sessions; any weakening of the §21.4 delegation
boundary.

---

### 21.59 v2.19 — Phase 6 complete (August 4, 2026)

Phase 6 — planning & the knowledge graph — closes with all three
sub-phase exit criteria met and attested:

1. **6.1 (August 2):** blueprint `260730-c6750c` materialized four
   dependency-ordered children on approval; children were claimable
   only in dependency order; the parent closed itself when the last
   child reached terminal state; decomposition-free specs remained
   byte-identical throughout.

2. **6.2 (August 2):** Phase 6.3 itself was planned entirely
   in-product — requirement `req-260802-72fc68` drafted by the
   planning agent and confirmed, blueprint approved at the ordinary
   spec gate with the `serves` link confirmed, four children
   materialized, transcripts archived as lineage — with the features
   tree migrated and its API/UI removed, and no external chat tool
   involved.

3. **6.3 (August 4):** the chain requirement → blueprint → task →
   work order → PR/commit range → evidence → verdict is queryable
   end to end from the API on live data; dispatched work receives
   link-derived context that task-scoped selection could not produce;
   staleness, REQ-n citation contracts, and the Blueprints/lineage
   surfaces operate on the graph. Hardening took two full
   review-fix-verify cycles — vocabulary unification (migration 057),
   PR-identity canonicalization (058/060), a non-destructive rebuild
   with per-rebuild reconciliation reporting, the lineage conformance
   suite, and per-PR CI — and closed with the first live rebuild:
   `{projected: 543, existing: 0, preserved_unregenerable: 53,
   unsupported: 192, ambiguous: 2}`, 596 links before and after,
   zero rows lost.

Three facts are recorded permanently rather than repaired, because
§16's append-only discipline outranks tidiness:

- Migration 054 fabricated 18 backdated `task.dependency_added`
  events (IDs 74871–74888). They remain in the corpus, are marked by
  a `lineage.historical_fabrication_recorded` audit event, and a
  guard test forbids any future migration from writing events.
- 53 historical task → pull-request edges are non-regenerable (their
  source events predate `repository`/`base_sha` capture). They are
  preserved verbatim on every rebuild and reported as
  `preserved_unregenerable`; lineage for those tasks terminates at
  canonical PR nodes without commit ranges.
- §18's untrusted-surface enumeration is extended in the body by this
  amendment to cover planning exploration reads, uploaded documents,
  and lineage-derived context — closing the documentation debt
  §21.50–§21.51 deferred.

docs/phase6-plan.md is historical. Per §21.58 change 9, **Phase 8 —
the desired-state document model — is now the active scope**, with
the outstanding pipeline-robustness work (the review-wedge task filed
from live operation) proceeding alongside as ordinary maintenance.
...
### 21.61 v2.21 — operating-surface simplification (August 7, 2026)

Adopted from operating the complete Phase 8 surface set. The
operator's judgment: the stage-grouped board is the only delivered
surface meeting the product's bar; the document and task surfaces
accreted machinery-truth — scattered drift chips, staleness banners,
pending-version stacks, an always-present assistant column — without
an information hierarchy. The direction is the document-as-hero shape
of the best contemporary factory tools. Five changes, all
presentation-layer; no gate, lifecycle, graph, or authority semantics
move:

1. **Document surfaces restructure.** Requirements and System Design
   present a category navigation tree and a document canvas. Each
   document carries exactly one **attention surface** consolidating
   drift signals, staleness, pending versions, and pending
   confirmations, each entry carrying its action; the scattered
   renderings of those signals retire from the surface. Alert noise
   is also a content matter: operators should keep `governs` scopes
   to what a document genuinely describes (guidance, not machinery).

2. **Lineage explorer.** Task, requirement, and System Design detail
   gain a corner affordance opening a right panel of related entities
   — grouped lists (work, documents, delivery, evidence) over the
   existing §16 lineage read API, linked to their surfaces. No graph
   visualization.

3. **Presentation parking** (parked, not retired; machinery, records,
   and deep links retained; reversal by future presentation
   amendment): the in-product planning chat surfaces — the assistant
   sidebar and the Planning tab (§21.57's presentation; its
   propose→confirm doctrine and goal machinery are untouched) — and
   the Blueprint-history navigation entry (§21.49's canonical routes
   and anchor rules are untouched; history reaches through task
   detail and deep links). The §21.60 local surface is the planning
   path while parked.

4. **Task surfaces consolidate.** Task creation relocates to the
   Tasks view; list rows open a permalinked right detail panel with
   the full route retained for deep links; the list is paginated with
   server-side filters — state, repository, free text, updated-at
   range, served requirement, governing design. The Board gains the
   same filter family and defaults to recently updated (last month),
   operator-adjustable. §6.3 barred fields stay barred.

5. **The §13.3 body gains the operating-surfaces paragraph** stating
   the exact surface set.

Delivery is planned as requirement documents plus a dependency-ordered
task set under §21.60. Out of scope, restated: any weakening of
propose→confirm, the §13.1 gates, drift semantics, or the §21.4
boundary; freehand editing; priority/assignee/phase fields.

### 21.62 v2.22 — the origin-task proposal carve-out (August 8, 2026)

Adopted from operating §21.61. Making the per-document attention
surface the *only* rendering of pending-version signals put the
decision on a different page from the work waiting on it: an implement
session proposes a System Design revision, and the task that raised it
renders nothing actionable. On task 260808-8f73d0 the operator watched
the task bounce through five review rounds while "Confirm version
17/18/19" sat on the System Design page, invisible from the task.

One change, presentation only:

1. **The origin task may render its own proposals.** A task's detail
   route MAY additionally render that task's own unresolved System
   Design proposals **on the documents that task carries as attached
   context** — the proposing document, the proposed version, a link to
   the document, and the existing operator-credentialed confirm
   action. §21.61 change 1 otherwise stands: the per-document
   attention surface remains the only *document-side* rendering, and
   drift and staleness rendering is unchanged and stays document-side.
   §13.3's operating-surfaces paragraph carries the carve-out.

The carve-out is scoped by origin and by attachment: a task renders
unresolved proposals it raised itself, on documents its own attached
context names, and no others. A proposal a task raised against a
document it does not carry stays on that document's attention surface,
where §21.61 left it. It moves no authority — the card invokes the
same operator-credentialed route the attention surface does, agents
still never confirm, and a resolved proposal simply stops matching, so
the card disappears with no new state. Out of scope: any weakening of
propose→confirm or the §13.1 gates; drift or staleness on task detail;
rendering another task's proposals; priority/assignee/phase fields.

---

*End of specification. v2.22 accepted August 8, 2026 — the v2.0
...
presentation. §21.54 accepts Phase 8 — deployment & multi-user: identity with admin
grants
...
(flywheel → 9, managed execution → 10,
enterprise → 11). §21.55 rejects the memory store outright — knowledge
promotes into maintained artifacts, never parks in volunteered rows —
rescoping Phase 7 to demand-triggered read-only recall and
resequencing Phase 8 to follow Phase 6 directly. §21.56 retires the
attempt-grouped history presentation, ratifying PR #207. §21.57 makes
the Requirements view document-first — the requirement as canvas, the
planning assistant as a scoped sidebar with guided actions, sessions
declaring a goal artifact — with freehand editing rejected in favor of
the propose→confirm path. §21.58 accepts Phase 8 — the desired-state
document model: durable documents as truth, tasks as reconciliation
with execution plans on the existing stage machinery, the spec
...
operator's direct pushes. §21.61 simplifies the operating surfaces:
document tree + canvas + one attention surface per document, the
lineage explorer panel, planning and blueprint-history presentation
parked, and consolidated task surfaces with shared filters. §21.62
opens one narrow carve-out in that sole-rendering rule: a task's own
detail additionally renders the System Design proposals that task
raised on the documents it carries, so the decision is visible from
the work waiting on it. The body (§§1–20) is the normative
