...
proposals with their confirm action; drift and staleness stay
document-side only. Task, requirement, and design detail expose an
on-demand lineage explorer — grouped related entities over the §16
read API, not a visualization. The in-product planning chat surfaces
...
1. **The origin task may render its own proposals.** A task's detail
   ...
   Design proposals — the proposing document, the proposed version,
   a link to the document, and the existing operator-credentialed
   confirm action. §21.61 change 1 otherwise stands: the per-document
...
The carve-out is scoped by origin: a task renders proposals it raised
itself and no others. It moves no authority — the card invokes the
...
raised, so the decision is visible from the work waiting on it. The
