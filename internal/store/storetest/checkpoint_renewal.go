...
func RunCheckpointRenewalConformance(t *testing.T, st store.Store, ctx context.Context) {
	for _, reason := range []string{
		...
		t.Run(reason, func(t *testing.T) {
			...
			const workerID = "worker-checkpoint"
			const sessionID = "session-checkpoint"
			if _, err := ClaimWorkOrder(ctx, st, orderID, core.WorkOrderClaim{
				WorkerID: workerID, ClaimantID: workerID, SessionID: sessionID,
				ClientToken: "checkpoint-secret", Lease: time.Minute, ExecutionTimeout: time.Hour,
			}); err != nil {
				t.Fatal(err)
			}
			if _, err := ReleaseWorkerClaim(ctx, st, orderID, workerID, core.WorkOrderRelease{
				SessionID: sessionID, Reason: reason, Cause: core.WorkOrderReleaseCauseOperatorAction,
				Outcome: core.WorkOrderOutcomeReleased,
			}); err != nil {
				t.Fatal(err)
			}
			renew := func(session string) error {
				_, err := taskops.ExecuteWorkOrder(ctx, st, taskID, core.WorkOrderCmdRenew, func(lease taskops.TaskLease) (core.WorkOrder, error) {
					return st.RenewWorkerClaimCommand(ctx, lease, orderID, core.WorkOrderClaimIdentity{WorkerID: workerID, ClaimantID: workerID, SessionID: session}, time.Minute)
				})
				return err
			}
			if err := renew(sessionID); !errors.Is(err, store.ErrWorkOrderReleasedAtCheckpoint) {
				t.Fatalf("same-session renew error=%v", err)
			}
			if err := renew("different-session"); !errors.Is(err, store.ErrWorkOrderClaimLost) {
