...
// logDispatchQueue is dispatchQueue on the event log. Enqueues run inside
// the caller's transaction by binding it to the log driver's context, so a
// lifecycle command's rows, its mirrored events, and its job commit
// together exactly as they did with River. The reconciliation reads keep
// their SQL over tasks and fold each candidate's job stream for the queue
// half of the answer.
//
...
// UseLogQueue switches the store's durable queue to the event log. The
// daemon must run the matching logqueue.Runtime for jobs to execute.
func (s *Store) UseLogQueue() {
	s.queue = newLogDispatchQueue(s.pool, s.logDriver())
}
