...
func (s *Store) ConfirmRequirementVersion(ctx context.Context, requirementID string, version int, expectedCurrentVersion ...int) (core.Requirement, core.RequirementVersion, error) {
	...
	err := s.inTx(ctx, func(tx pgx.Tx, q *db.Queries) error {
		...
		retiredRows, err := tx.Query(ctx, `UPDATE requirement_versions
			SET retired=true, retired_by=$4, retired_at=$5, retired_by_version=$3,dismissal_note=NULLIF($6,'')
			WHERE workspace_id=$1 AND requirement_id=$2 AND version<$3
			  AND NOT confirmed AND NOT retired
			RETURNING version`, workspace(ctx), requirementID, version, actor.ID, now, store.DocumentDismissalNote(ctx))
		if err != nil {
			return err
		}
		...
		for _, retiredVersion := range retiredVersions {
			if err = insertRequirementEvent(ctx, q, "requirement.version_retired", store.DocumentDismissalEventPayload(ctx, map[string]any{
...
func (s *Store) DismissRequirementVersion(ctx context.Context, requirementID string, version int) (core.Requirement, core.RequirementVersion, error) {
	...
	err := s.inTx(ctx, func(tx pgx.Tx, q *db.Queries) error {
		var err error
		requirement, dismissed, err = dismissRequirementVersionTx(ctx, tx, q, requirementID, version)
		return err
	})
	return requirement, dismissed, err
}

func dismissRequirementVersionTx(ctx context.Context, tx pgx.Tx, q *db.Queries, requirementID string, version int) (core.Requirement, core.RequirementVersion, error) {
	...
	err := func() error {
		...
		if _, err = tx.Exec(ctx, `UPDATE requirement_versions
			SET retired=true, retired_by=$4, retired_at=$5, retired_by_version=NULL,dismissal_note=NULLIF($6,'')
			WHERE workspace_id=$1 AND requirement_id=$2 AND version=$3`,
			workspace(ctx), requirementID, version, actor.ID, now, store.DocumentDismissalNote(ctx)); err != nil {
			return err
		}
...
func (s *Store) FinalizePlanningSession(ctx context.Context, request store.PlanningFinalizeRequest) (core.PlanningSession, error) {
	...
	err := s.inTx(ctx, func(tx pgx.Tx, q *db.Queries) error {
		...
		if existing.RequirementContextID != "" && request.TaskID != "" {
			if _, err := validateTaskContextTx(ctx, tx, workspace(ctx), store.TaskContextInput{RequirementIDs: []string{existing.RequirementContextID}}); err != nil {
				return err
			}
		}
		now := time.Now().UTC()
