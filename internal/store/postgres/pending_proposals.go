...
func (s *Store) ListPendingAuthorityProposalsForTask(ctx context.Context, taskID string) ([]core.PendingProposal, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM (
			...
			FROM system_design_versions v
			JOIN system_designs d ON d.workspace_id=v.workspace_id AND d.id=v.document_id
			WHERE v.workspace_id=$1 AND v.origin_task_id=$2 AND d.archived_at IS NULL AND NOT v.confirmed AND NOT v.dismissed
			UNION ALL
			SELECT v.requirement_id,r.title,'requirement' AS tier,v.version,
				'task',v.origin_task_id,'','',v.created_at
			FROM requirement_versions v
			JOIN requirements r ON r.workspace_id=v.workspace_id AND r.id=v.requirement_id
			WHERE v.workspace_id=$1 AND v.origin_task_id=$2 AND r.archived_at IS NULL AND NOT v.confirmed AND NOT v.retired
			  AND v.version > coalesce(r.current_version,0)
			UNION ALL
			SELECT d.id,d.statement,'decision' AS tier,NULL::integer,
				'task',d.origin_task_id,'','',d.created_at
			FROM decisions d
			WHERE d.workspace_id=$1 AND d.origin_task_id=$2 AND d.status='proposed'
		) pending
		ORDER BY proposed_at,tier,proposal_id,version NULLS FIRST`, workspace(ctx), taskID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
