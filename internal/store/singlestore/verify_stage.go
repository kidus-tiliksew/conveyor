...
func verifyReviewReadyTx(ctx context.Context, tx *sql.Tx, task core.Task) (bool, error) {
	...
	var ready bool
	err := tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM work_orders WHERE workspace_id=? AND task_id=? AND stage='verify' AND state='completed' AND head_sha=? AND head_sha<>'')`, documentWorkspace(ctx), task.ID, core.VerifyStageHead(task)).Scan(&ready)
	return ready, err
}
