...
func TestTaskSetupChangeSingleStoreScopesExclusionToExecutingAttempts(t *testing.T) {
	...
	if _, err = st.BootstrapWorkspaceConfig(ctx, &config.Config{Workspace: workspace, Database: config.Database{Backend: "singlestore"}, Repos: []config.Repo{{Name: "repo", Base: "main"}}}); err != nil {
		t.Fatal(err)
	}
...
func TestTaskSetupChangeSingleStoreReaggregatesRetainedAndReplacementSeats(t *testing.T) {
	st := integrationStore(t)
	var err error
