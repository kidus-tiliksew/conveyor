...
export function WorkspacePage() {
  const token = useOperatorToken(); const queryClient = useQueryClient(); const { data: snapshot } = useWorkspace()
  const query = useQuery({ queryKey: ['workspace-config', token], queryFn: () => fetchWorkspaceConfig(token), enabled: Boolean(token) })
  const [draft, setDraft] = useState<WorkspaceConfigDocument | null>(null); const [saved, setSaved] = useState('')
  useEffect(() => { if (query.data) setDraft(structuredClone(query.data.document)) }, [query.data])
  const save = useMutation({ mutationFn: () => updateWorkspaceConfig(token, draft!, query.data!.version), onSuccess: (receipt) => { queryClient.setQueryData(['workspace-config', token], receipt); void queryClient.invalidateQueries({ queryKey: ['workspace'] }); setDraft(structuredClone(receipt.document)); setSaved(`Recorded config.updated event ${receipt.event_id} · version ${receipt.version}`) } })
  return <div className="h-full overflow-y-auto"><div className="mx-auto max-w-5xl px-6 py-8">
    <div className="flex items-start justify-between"><div><h1 className="text-xl font-semibold">Workspace</h1><p className="mt-1 text-sm text-muted">Repositories and per-stage execution routing.</p></div>{draft && <Button disabled={save.isPending} onClick={() => save.mutate()}><Save />Save</Button>}</div>
...
function Editor({ draft, setDraft }: { draft: WorkspaceConfigDocument; setDraft: (value: WorkspaceConfigDocument) => void }) {
  ...
  return <div className="mt-6 space-y-4">
    <Card><CardHeader><CardTitle>Workspace</CardTitle><Badge variant="mono">{draft.workspace}</Badge></CardHeader><CardContent><Field label="Maximum bounce rounds"><Input type="number" min={1} value={draft.max_bounces} onChange={(event) => update({ max_bounces: Number(event.target.value) })} /></Field></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle>Stage routing</CardTitle><span className="text-xs text-faint">Triage/spec are fixed in-process; implement is fixed MCP.</span></CardHeader><CardContent className="space-y-3">{stageOrder.map((stage) => { const route = draft.routing.stages[stage]; if (!route) return null; return <div key={stage} className="grid gap-3 rounded-lg border border-border p-3 md:grid-cols-5"><Field label="Stage"><strong className="block py-2 text-sm">{stage}</strong></Field><Field label="Execution"><select className="h-9 w-full rounded-md border border-border bg-background px-2 text-sm" value={route.execution} disabled={stage !== 'review'} onChange={(event) => updateRoute(stage, { execution: event.target.value as 'mcp' | 'in_process' })}><option value="in_process">in_process</option><option value="mcp">mcp</option></select></Field><Field label="Model"><Input value={route.model} onChange={(event) => updateRoute(stage, { model: event.target.value })} /></Field><Field label="Budget USD"><Input type="number" min={0} step="0.01" value={route.budget_usd} onChange={(event) => updateRoute(stage, { budget_usd: Number(event.target.value) })} /></Field><Field label="Timeout"><Input value={route.timeout} onChange={(event) => updateRoute(stage, { timeout: event.target.value })} /></Field></div> })}</CardContent></Card>
  </div>
}
