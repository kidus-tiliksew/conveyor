...
export function SetupCard({ document, setup, index, expanded, onToggle, workerReady, workerReason, setDraft, onDuplicate, onDelete }: {
  ...
  const implementSummary = [
    ...
    settings.implementation.model_policy === 'explicit' ? settings.implementation.model || 'no model' : 'harness default',
      ...
      {expanded && (
        <div className="border-t border-border">
          ...
          <div className="border-b border-border px-4 py-4">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <Field label="Harness">
                <Select aria-label="Implementation harness" value={settings.implementation.harness} onChange={(event) => updateImplementation({ harness: event.target.value, model: '' })}>
                  <option value="">Select harness</option>
                  {document.harnesses.map((harness) => <option key={harness.name} value={harness.name}>{harness.name}</option>)}
                </Select>
              </Field>
              <Field label="Model policy">
                <Select aria-label="Implementation model policy" value={settings.implementation.model_policy} onChange={(event) => updateImplementation({ model_policy: event.target.value as 'explicit' | 'harness_default', model: '' })}>
              ...
              {settings.implementation.model_policy === 'explicit' ? (
                <Field label="Model"><Input aria-label="Implementation model" className="font-mono" value={settings.implementation.model ?? ''} placeholder="Provider model ID" onChange={(event) => updateImplementation({ model: event.target.value })} /></Field>
              ) : (
                <Field label="Default sentinel">
                  <Select aria-label="Implementation default sentinel" value={settings.implementation.model ?? ''} onChange={(event) => updateImplementation({ model: event.target.value })}>
                    <option value="">Omit model arguments</option>
                    {(implementationHarness?.default_model_sentinels ?? []).map((sentinel) => <option key={sentinel} value={sentinel}>{sentinel}</option>)}
                  </Select>
                </Field>
              )}
              <Field label="Reasoning effort">
                <Select aria-label="Implementation reasoning effort" value={settings.implementation.effort ?? ''} onChange={(event) => updateImplementation({ effort: (event.target.value || undefined) as typeof settings.implementation.effort })}>
              ...
              <Field label="Time limit"><Input aria-label="Implementation timeout" className="max-w-28" value={settings.implementation.timeout} onChange={(event) => updateImplementation({ timeout: event.target.value })} /></Field>
            </div>
          </div>
          ...
          <div className="border-b border-border px-4 py-4">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <Field label="Time limit"><Input aria-label="Review timeout" value={settings.review.timeout} onChange={(event) => updateReviewExecution({ timeout: event.target.value })} /></Field>
              {settings.review.execution === 'mcp' && reviewNeedsFallback && (
                <Field label="Fallback harness">
                  <Select aria-label="Review fallback harness" value={settings.review.fallback_harness ?? ''} onChange={(event) => updateReviewExecution({ fallback_harness: event.target.value || undefined })}>
            ...
            <p className="mt-2 text-xs text-faint">{reviewNeedsFallback ? 'Seats without a harness use the review fallback; only that fallback is validated and health-gated.' : 'Every seat is explicitly routed.'}</p>
            <div className="mt-3 space-y-2">
              {seats.map((seat, seatIndex) => (
                <div key={seatIndex} className="grid items-end gap-3 rounded-md border border-border px-3 py-2.5 md:grid-cols-[auto_1fr_1fr_1fr_auto]">
                  <span className="pb-2 text-xs text-faint">Seat {seatIndex + 1}</span>
                  <Field label="Pinned model"><Input aria-label="Pinned model" className="font-mono" value={seat.model} onChange={(event) => updateSeat(seatIndex, { model: event.target.value })} /></Field>
                  <Field label="Harness">
                    <Select aria-label={`Seat ${seatIndex + 1} harness`} value={seat.harness ?? ''} onChange={(event) => updateSeat(seatIndex, { harness: event.target.value || undefined })}>
                      <option value="">Use review fallback</option>
                      {document.harnesses.map((harness) => <option key={harness.name} value={harness.name}>{harness.name}</option>)}
                    </Select>
                  </Field>
                  <Field label="Reasoning effort">
                    <Select aria-label={`Seat ${seatIndex + 1} reasoning effort`} value={seat.effort ?? ''} onChange={(event) => updateSeat(seatIndex, { effort: (event.target.value || undefined) as WorkspaceReviewSeat['effort'] })}>
                      <option value="">Harness default</option>
                      <option value="low">Low</option>
                      <option value="medium">Medium</option>
                      <option value="high">High</option>
                  ...
                  <Button aria-label={`Remove review seat ${seatIndex + 1}`} size="icon" variant="ghost" className="mb-0.5 hover:text-failure" onClick={() => updateSetup({ review: { seats: seats.filter((_, i) => i !== seatIndex) } })}><Trash2 /></Button>
                </div>
              ))}
            </div>
            <Button size="sm" variant="secondary" className="mt-3" onClick={() => updateSetup({ review: { seats: [...seats, { model: settings.review.fallback_model ?? '', harness: undefined, effort: undefined }] } })}><Plus />Add seat</Button>
          </div>

          <div className="flex flex-wrap items-end gap-3 bg-surface px-4 py-3">
            <Field label="Setup name"><Input aria-label="Setup name" value={setup.name} onChange={(event) => updateSetup({ name: event.target.value })} /></Field>
            <div className="ml-auto flex gap-2">
              <Button size="sm" variant="secondary" aria-label={`Duplicate ${setup.name}`} onClick={onDuplicate}><Copy />Duplicate</Button>
              <Button size="sm" variant="secondary" aria-label={`Set ${setup.name} as default`} disabled={isDefault} onClick={() => setDraft({ ...document, default_setup: setup.name, execution_settings: setup.execution_settings, review: setup.review })}>Set as default</Button>
              <Button size="sm" variant="destructive" aria-label={`Delete ${setup.name}`} disabled={document.setups.length <= 1} onClick={onDelete}><Trash2 />Delete</Button>
            </div>
          </div>
        </div>
      )}
...
function Stage({ name, model, meta, connected }: { name: string; model: string; meta: string; connected?: boolean }) {
  return (
    <div className={cn('relative min-w-40 flex-1 rounded-md border border-border bg-surface px-3 py-2', connected && "before:absolute before:-left-4 before:top-1/2 before:h-px before:w-4 before:bg-edge before:content-['']")}>
      <p className="text-[10px] font-semibold uppercase tracking-wider text-faint">{name}</p>
      <p className="truncate font-mono text-xs text-foreground" title={model}>{model}</p>
...
function GroupTitle({ title, note }: { title: string; note: string }) {
  return <h4 className="mb-3 text-sm font-semibold">{title} <span className="font-normal text-faint">— {note}</span></h4>
}
