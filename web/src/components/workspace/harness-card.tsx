    ...
    <div className="rounded-md border border-border bg-card">
      ...
      >
        <span className="font-mono text-sm font-semibold">
          {harness.name || <span className="font-sans font-normal italic text-faint">unnamed harness</span>}
        </span>
        <Badge>{TRANSPORT_LABELS[harness.mcp_transport ?? 'json_file']}</Badge>
        {probe ? (
          <Badge variant={probe.healthy ? 'positive' : 'attention'}>
            {probe.healthy ? 'Probe healthy' : `Probe failing${probe.message ? ` — ${probe.message}` : ''}`}
          </Badge>
        ) : (
      ...
      {expanded && (
        <div className="border-t border-border">
          <div className="grid gap-3 px-4 py-4 md:grid-cols-2">
            <Field label="Name">
              <Input
                aria-label={`Harness ${index + 1} name`}
                value={harness.name}
              ...
              <Select
                ...
                onChange={(event) => {
                  ...
                  onChange({
                    ...
                    mcp_attachment: transport === 'environment' ? harness.mcp_attachment || 'conveyor' : undefined,
                  })
                }}
              >
            ...
            {environment && (
              <Field label="MCP attachment">
                <Input
                  aria-label={`MCP attachment for ${displayName}`}
                  value={harness.mcp_attachment ?? ''}
                  placeholder="conveyor"
                  onChange={(event) => onChange({ mcp_attachment: event.target.value })}
                />
              </Field>
            )}
            <div className="md:col-span-2">
              <Field
                label="Command argv"
                hint="Run as an argv array — no shell. Type or paste a command; it splits on spaces. Click an argument to edit it."
              >
                <ArgvInput
                  label="Command argv"
            ...
            <Field label="Model argv" hint="Appended when a model is pinned; {model} is replaced with the model ID.">
              <ArgvInput
                label="Model argv"
                value={harness.model_args ?? []}
                onChange={(value) => onChange({ model_args: value })}
                placeholder="None"
              />
            </Field>
          </div>
          <details className="border-t border-border px-4 py-3">
            <summary className="cursor-pointer select-none text-sm font-medium text-muted hover:text-foreground">
              ...
              <span className="font-normal text-faint">
                — effort flags, model detection, health and stall supervision
              </span>
            </summary>
            <div className="grid gap-3 pb-1 pt-4 md:grid-cols-2">
              <Field label="Low effort argv">
                <ArgvInput
                  label="Low effort argv"
                  value={harness.effort_args?.low ?? []}
                  onChange={(value) => onEffortChange('low', value)}
                  placeholder="Not supported — effort Low is rejected for this harness"
                />
              </Field>
              <Field label="Medium effort argv">
                <ArgvInput
                  label="Medium effort argv"
                  value={harness.effort_args?.medium ?? []}
                  onChange={(value) => onEffortChange('medium', value)}
                  placeholder="Not supported"
                />
              </Field>
              <Field label="High effort argv">
                <ArgvInput
                  label="High effort argv"
                  value={harness.effort_args?.high ?? []}
                  onChange={(value) => onEffortChange('high', value)}
                  placeholder="Not supported"
                />
              </Field>
              <Field
                label="Default model sentinels"
                hint="Model names that mean “let the agent pick”. Leave empty unless the CLI has magic values."
              >
                <ArgvInput
                  label="Default model sentinels"
                  value={harness.default_model_sentinels ?? []}
                  onChange={(value) => onChange({ default_model_sentinels: value })}
                  placeholder="None"
                />
              </Field>
              <Field
                label="Probe argv"
                hint="Cheap health check the worker runs to confirm the CLI is installed and launchable."
              >
                <ArgvInput
                  label="Probe argv"
                  value={harness.probe_command}
                  onChange={(value) => onChange({ probe_command: value })}
                />
              </Field>
              <Field label="Probe timeout">
                <Input
                  className="max-w-28"
                  value={harness.probe_timeout}
                  onChange={(event) => onChange({ probe_timeout: event.target.value })}
                />
              </Field>
              <Field
                label="Stall timeout"
                hint="Stop a child after this long without stdout or stderr. Use 0 only for intentionally silent harnesses."
              >
                <Input
                  className="max-w-28"
                  value={harness.stall_timeout ?? '10m'}
                  onChange={(event) => onChange({ stall_timeout: event.target.value })}
                />
              </Field>
            </div>
          </details>
          <div className="flex justify-end border-t border-border bg-surface px-4 py-2.5">
            <Button size="sm" variant="destructive" aria-label={`Remove ${displayName}`} onClick={onRemove}>
              <Trash2 />
              Remove harness
            </Button>
          </div>
