...
function PanelBody({ item }: { item: ActivityItem }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {item.work_orders?.map((order) => <div key={order.id} className="rounded-lg border border-border bg-surface p-3"><div className="flex items-center gap-2"><Badge variant="accent">{order.stage} work order</Badge><Badge variant={order.state === 'claimed' ? 'positive' : 'mono'}>{order.state}</Badge></div><p className="mt-2 text-xs text-muted">{order.claimed_by ? `Claimed by ${order.claimed_by}${order.agent ? ` · ${order.agent}` : ''}${order.model ? ` · ${order.model}` : ''}` : 'Waiting for an operator-owned agent over MCP.'}</p>{order.lease_expires_at && <p className="mt-1 text-[11px] text-faint">Lease expires {new Date(order.lease_expires_at).toLocaleString()} · usage self-reported</p>}{order.progress && <p className="mt-2 text-xs">{order.progress}</p>}</div>)}
