  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className={`flex shrink-0 items-center gap-3 border-b border-border ${gutter} py-3`}>
        ...
        {!sidebar && session.model && <Badge variant="mono">{session.model}{session.effort ? ` · ${session.effort}` : ''}</Badge>}
      ...
      {sessionArtifacts.length > 0 && (
        <section className={`flex shrink-0 flex-wrap items-center gap-2 border-b border-border bg-surface/50 ${gutter} py-2`} aria-label="Planning attachments">
      ...
      {session.status === 'active' && (
        ...
        >
          <div className={`rounded-xl border border-edge bg-card p-2 shadow-sm focus-within:outline-2 focus-within:outline-primary ${sidebar ? '' : 'mx-auto max-w-3xl'}`}>
          ...
          {upload.error && <p className={`mt-2 text-xs text-failure ${sidebar ? '' : 'mx-auto max-w-3xl'}`}>{errorMessage(upload.error, 'Could not attach that file.')}</p>}
          {!token && <p className={`mt-2 text-xs text-attention ${sidebar ? '' : 'mx-auto max-w-3xl'}`}>Set the operator token in Settings to plan.</p>}
        </form>
      )}
