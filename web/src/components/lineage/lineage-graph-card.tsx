...
const edgeLabels: Record<string, string> = {
  produced_requirement: 'produced requirement',
  produced_blueprint: 'produced blueprint',
  serves: 'serves',
  versions: 'has version',
  supersedes: 'supersedes',
  materializes: 'materializes child',
  depends_on: 'depends on',
  dispatches: 'dispatches as',
  submitted_as: 'submitted as',
  submitted_range: 'submitted commit range',
  merged_range: 'merged commit range',
    ...
    <Card>
      <CardHeader>
        ...
        <span className="flex items-center gap-1.5">
          ...
          {(graph.omitted_nodes ?? 0) + (graph.omitted_links ?? 0) > 0 && (
            <Badge variant="attention">{(graph.omitted_nodes ?? 0) + (graph.omitted_links ?? 0)} omitted</Badge>
          )}
          <Badge variant="mono">
          ...
          <details>
            ...
            <ol className="mt-3 space-y-2">
              {graph.links.map((link) => (
                <li
                  key={`${link.created_by_event_id}-${link.src_type}-${link.src_id}-${link.kind}`}
                  className="grid gap-1 rounded-md border border-border bg-surface px-3 py-2 text-xs sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] sm:items-center"
                >
                  <LineageNode
                    type={link.src_type}
                    id={link.src_id}
                    label={labels.get(`${link.src_type}:${link.src_id}`)}
                  />
                  <span className="flex items-center gap-1 font-mono text-[10px] text-faint">
                    <ArrowRight className="size-3" /> {humanize(link.kind)}
                  </span>
                  <LineageNode
                    type={link.dst_type}
                    id={link.dst_id}
                    label={labels.get(`${link.dst_type}:${link.dst_id}`)}
                  />
                </li>
              ))}
            </ol>
...
function LineageNode({ type, id, label }: { type: LineageNodeType; id: string; label?: string }) {
  return (
    <span className="min-w-0">
      <span className="block text-[10px] uppercase tracking-wide text-faint">{nodeLabels[type] ?? humanize(type)}</span>
...
function humanize(value: string) {
  return edgeLabels[value] ?? value.replaceAll('_', ' ')
}
