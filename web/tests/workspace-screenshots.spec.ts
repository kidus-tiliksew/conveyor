...
const baseDocument = {
  ...
  routing: { stages: {} },
  harnesses: [
    { name: 'codex', mcp_transport: 'toml_override', command: ['codex', 'exec', '{prompt}', '--config', '{mcp_config}'], model_args: ['--model', '{model}'], effort_args: { medium: ['--config', 'model_reasoning_effort="medium"'], high: ['--config', 'model_reasoning_effort="high"'] }, probe_command: ['codex', '--version'], probe_timeout: '10s' },
    { name: 'claude', mcp_transport: 'json_file', command: ['claude', '-p', '{prompt}', '--mcp-config', '{mcp_config}'], model_args: ['--model', '{model}'], effort_args: { medium: ['--effort', 'medium'], high: ['--effort', 'high'] }, probe_command: ['claude', '--version'], probe_timeout: '10s' },
    { name: 'grok', mcp_transport: 'json_file', command: ['grok', '--single', '{prompt}', '--mcp-config', '{mcp_config}'], model_args: ['--model', '{model}'], effort_args: { low: ['--reasoning-effort', 'low'], medium: ['--reasoning-effort', 'medium'], high: ['--reasoning-effort', 'high'] }, probe_command: ['grok', '--version'], probe_timeout: '10s' },
  ],
...
baseDocument.setups = [
  ...
  { name: 'claude-heavy', execution_settings: { ...baseDocument.execution_settings, implementation: { model: 'claude-fable-5', model_policy: 'explicit', harness: 'claude', timeout: '2h' } }, review: baseDocument.review },
]
...
async function mockAPIs(page: Page) {
  ...
  await page.route('**/v1/**', async (route: Route) => {
    ...
    if (url.pathname === '/v1/workspaces') { await route.fulfill({ json: [{ id: 'conveyor', name: 'Conveyor', config_version: 1, created_at: '2026-07-17T00:00:00Z' }] }); return }
    if (url.pathname === '/v1/workspace/config') { await route.fulfill({ json: { document: baseDocument, version: 1 } }); return }
    if (url.pathname === '/v1/workers') {
      await route.fulfill({ json: {
        workers: [{ id: 'wrk-1', workspace: 'conveyor', name: 'kidus-mbp', last_seen_at: new Date().toISOString(), lease_expires_at: new Date(Date.now() + 60000).toISOString(), created_at: '2026-07-17T00:00:00Z', probes: [
          { harness: 'codex', healthy: true, checked_at: new Date().toISOString() },
          { harness: 'grok', healthy: true, checked_at: new Date().toISOString() },
          { harness: 'claude', healthy: false, message: 'claude: command not found', checked_at: new Date().toISOString() },
        ] }],
        auto_available: true,
        setup_serviceability: { 'setup-1': { auto_available: true }, 'claude-heavy': { auto_available: false, auto_unavailable_reason: 'claude probe failing' } },
      } })
      return
    }
