...
test('monitor page renders health, deduplication, task links, and drift age', async ({ page }) => {
  ...
  await page.route('**/v1/workspaces', (route) => route.fulfill({ json: [{ id: 'demo', name: 'Demo', config_version: 1 }] }))
  ...
  await page.route('**/v1/monitor?**', (route) => route.fulfill({ json: {
    workspace_id: 'demo',
    enabled: true,
    last_successful_observation: '2026-07-28T12:00:00Z',
    observations: [{
      workspace_id: 'demo', repository: 'conveyor', kind: 'post_merge_failure',
      occurrence_id: 'check:77:attempt:1', source_url: 'https://example.test/check/77',
      task_id: 'monitor-task', task_outcome: 'created', state: 'deduplicated',
      deduplicated_count: 2, created_at: '2026-07-28T12:00:00Z', updated_at: '2026-07-28T12:02:00Z',
    }],
    drift: [{
      id: 'direct_push:conveyor:abc', workspace_id: 'demo', repository: 'conveyor',
      kind: 'direct_push', source_url: 'https://example.test/commit/abc', commit_sha: 'abc',
      task_id: 'drift-task', detected_at: '2026-07-28T11:00:00Z',
    }],
    drift_count: 1,
