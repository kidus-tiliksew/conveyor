...
func TestWorkspaceGitHubAppResolverUsesInstallationIdentity(t *testing.T) {
	...
	resolve := func(c context.Context, repo string) (context.Context, error) {
		workspace, _ := store.WorkspaceFromContext(c)
		_, e := workspaceMonitorGitHubRunner(c, st, workspace, repo, client)
		return c, e
	}
	run, e := workspaceMonitorGitHubRunner(ctx, st, "demo", "org/repo", client)
	if e != nil {
		t.Fatal(e)
	}
	_, err = run(ctx, "api", "/repos/org/repo/pulls/1/files")
