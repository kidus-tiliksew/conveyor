...
func workerLaunchPrompt(order core.WorkOrder, workspace, sessionID string) string {
	...
	return prompt + " Use the Conveyor MCP server. First call get_work_order with that exact session_id for the approved contract. Immediately after get_work_order returns, call report_progress with a concise, plain-language summary of what the work order is about and what you will do next. You may also print the announcement. Make this announcement before running checkout, inspecting files, or starting implementation. The announcement is informational: continue automatically without asking for confirmation, waiting for a user response, or pausing. After conveyor checkout succeeds, call report_progress naming the worktree path and base commit. After completing each numbered contract item or approved-plan step, call report_progress naming the item or step and the files changed. Before submit_for_review, call report_progress with the validation commands run. Keep each progress message under a few sentences. Then complete the standard implement lifecycle, reporting and exiting after submit_for_review succeeds without polling await_review."
}
