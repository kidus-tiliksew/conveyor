...
func (c *client) reportDispatchContinuationLaunchContext(ctx context.Context, credential string, item workerservice.DispatchOrder, sessionID, mode, reason string) error {
	payload, _ := json.Marshal(map[string]any{
		"jsonrpc": "2.0", "id": 1, "method": "tools/call",
		"params": map[string]any{
			"name": "report_continuation_launch",
			"arguments": map[string]any{
				"workspace_id": c.workspace, "work_order_id": item.Order.ID, "session_id": sessionID,
	...
	if envelope.Result.IsError {
		message := "report_continuation_launch failed"
		if len(envelope.Result.Content) > 0 && strings.TrimSpace(envelope.Result.Content[0].Text) != "" {
			message = strings.TrimSpace(envelope.Result.Content[0].Text)
		}
		return errors.New(message)
