...
A factory is the industrial answer to exactly this shape of problem. You do
not inspect every screw; you fix the process so that inspection happens at
the points where mistakes are expensive, and you make every unit traceable
...
One boundary keeps the whole thing honest about what it is: Conveyor
coordinates work, it does not execute it. Agents run on your machines with
...
holds your model keys. If you deleted Conveyor tomorrow, the repository,
...
The graph is
a projection: you can delete it and rebuild it from the ledger.
...
The graph is a working memory. When an agent
claims a work order, a traversal of the graph supplies its context:
...
The factory's past work is how its future work gets its bearings, which is
also why Conveyor has no separate memory store: durable knowledge belongs in
...
Document confirmation is
operator-only at every light level; agents propose and keep working, and
nothing they propose becomes authority on its own. Misalignment signals are
likewise never auto-resolved. 

The darkest configuration Conveyor supports
is a factory that writes, reviews, and merges its own code all night, and
still cannot change what it is supposed to be building without a human
saying so in the morning.
