...
- Most files in `internal/store/postgres/db/` are generated by sqlc and must not
  be edited manually. `lineage_manual.go` is the intentional hand-maintained
  exception: it mirrors `queries/control_plane.sql` because the templated
  migrations are not parseable by sqlc outside the build-time validation path.
...
- There is no memory store: `store_memory` and memory rows are rejected by
